...
import numpy as np
import requests
from splight_lib.models import SplightDatabaseBaseModel
...
class OpenElevationClient:
    ...
    def get_altitude(self, lat: float, lng: float) -> float:
        response = requests.get(
            self.url, params={"locations": f"{lat},{lng}"}, timeout=5
        )
...
class Tower:
    def __init__(
        self,
        full_asset: SplightDatabaseBaseModel = None,
        client: OpenElevationClient = None,
    ):
    ...
    def create_from(
        self, full_asset: SplightDatabaseBaseModel, client: OpenElevationClient
    ):
    ...
    def span_length_from(self, other_tower: Tower) -> float:
        ...
        diff_in_altitude = (self.location.alt + self.line_height) - (
            other_tower.location.alt + other_tower.line_height
        )
...
def get_location(
    asset: SplightDatabaseBaseModel, client: OpenElevationClient
) -> Location:
...
def get_metadata(metadata_name: str, asset_id: str) -> str:
    ...
    assert response.status_code == 200, (
        f"Failed to get metadata for {metadata_name} on asset {asset_id}"
    )
