...
static bool interrogationHandler(void *parameter, IMasterConnection connection,
    ...
    if (qoi == 20) { /* only handle station interrogation */
        ...
        /* The CS101 specification only allows information objects without
         * timestamp in GI responses */

        CS101_ASDU newAsdu = CS101_ASDU_create(
            alParams, false, CS101_COT_INTERROGATED_BY_STATION, 0, 1, false,
            false);

        InformationObject io = (InformationObject)MeasuredValueScaled_create(
            NULL, 100, -1, IEC60870_QUALITY_GOOD);

        CS101_ASDU_addInformationObject(newAsdu, io);

        CS101_ASDU_addInformationObject(
            newAsdu,
            (InformationObject)MeasuredValueScaled_create(
        ...
        newAsdu = CS101_ASDU_create(alParams, false,
                                    CS101_COT_INTERROGATED_BY_STATION, 0, 1,
                                    false, false);

        io = (InformationObject)SinglePointInformation_create(
            NULL, 104, true, IEC60870_QUALITY_GOOD);
        ...
        newAsdu =
            CS101_ASDU_create(alParams, true, CS101_COT_INTERROGATED_BY_STATION,
                              0, 1, false, false);

        CS101_ASDU_addInformationObject(
            newAsdu, io = (InformationObject)SinglePointInformation_create(
                         NULL, 300, true, IEC60870_QUALITY_GOOD));
        CS101_ASDU_addInformationObject(
            newAsdu,
            (InformationObject)SinglePointInformation_create(
...
int main(int argc, char **argv) {
    ...
    int tcpPort = CS104_DEFAULT_PORT;
    ...
    // Update Port (only if needed)
    if (tcpPort != CS104_DEFAULT_PORT) CS104_Slave_setLocalPort(slave, tcpPort);
     ...
     * you can also modify the parameters here when default parameters are not
     * to be used */
    CS101_AppLayerParameters alParams =
    ...
    while (running) {
        ...
        CS101_ASDU newAsdu = CS101_ASDU_create(
            alParams, false, CS101_COT_PERIODIC, 0, 1, false, false);

        InformationObject io = (InformationObject)MeasuredValueScaled_create(
            NULL, 110, scaledValue, IEC60870_QUALITY_GOOD);
