...
class Master(Node):
    ...
    def __init__(
        ...
        self._stack_config: asiodnp3.MasterStackConfig = self._configure_stack(
        ...
        self._master = self._channel.AddMaster(
            "master", self._soe_handler, self._application, self._stack_config
        )
    ...
    def _configure_stack(
        local_addr: int, remote_addr: int
    ...
    def scan(self, group: int, variation: int, index: int, count: int = 1) -> None:
        ...
        """
        self._logger.debug(
            f"Creating scan to group {group}, variation {variation}, index {index}"
        )
    ...
    def operate(self, group: int, index: int, value: VarData) -> None:
        ...
        if group == 41:
            ...
            try:
                command = opendnp3.ControlRelayOutputBlock(
                    ...
                    else opendnp3.ControlCode.PULSE_OFF
                )
        ...
        self._logger.success(
            "Command operated on group {dnp_var.group}, index {dnp_var.index}"
        )
