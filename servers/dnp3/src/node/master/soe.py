...
from utils.types import IndexedDnpData, Variable
...
class SOEHandler(opendnp3.ISOEHandler):
    ...
    def _var_handler(self, gv: Tuple[int, int], ivar: IndexedDnpData) -> None:
        ...
        if isinstance(ivar.value, opendnp3.TimeAndInterval):
            self._logger.log(
                f"Update: {gv[0]} | {gv[1]} | {ivar.index} | {ivar.value.time.value}"
            )
        elif isinstance(ivar.value, opendnp3.OctetString):
            self._logger.error(
                "Error creating variable from update: Unsoported OctetStrin"
            )
            return
        else:
            self._logger.log(
                f"Update: {gv[0]} | {gv[1]} | {ivar.index} | {ivar.value.value}"
            )
