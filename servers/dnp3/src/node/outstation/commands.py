...
class CommandHandler(opendnp3.ICommandHandler):
    ...
    def Operate(
        self, command: DnpCommand, index: int, op_type: opendnp3.OperateType
        ...
        if (
            op_type == opendnp3.OperateType.SELECT_BEFORE_OPERATE
            and not self._command_is_selected(command, index)
        ):
