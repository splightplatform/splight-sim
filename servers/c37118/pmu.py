...
class Pmu(object):
    ...
    def set_data_rate(self, data_rate):
        ...
        self.logger.info(
            "[%d] - PMU reporting data rate changed.", self.cfg2.get_id_code()
        )
    ...
    def send(self, frame):
        if not isinstance(frame, CommonFrame) and not isinstance(frame, bytes):
            raise PmuError(
                "Invalid frame type. send() method accepts only frames or raw bytes."
            )
    ...
    def send_data(
        ...
        if phasors and self.cfg2.get_num_pmu() > 1:  # Check if multistreaming:
            if not (
                self.cfg2.get_num_pmu()
                == len(self.cfg2.get_data_format())
                == len(phasors)
            ):
                raise PmuError(
            ...
            for i, df in self.cfg2.get_data_format():
                if not df[1]:  # Check if phasor representation is integer
                    phasors[i] = map(
                        lambda x: int(x / (0.00001 * self.cfg2.get_ph_units()[i])),
                        phasors[i],
                    )
        elif not self.cfg2.get_data_format()[1]:
            phasors = map(
                lambda x: int(x / (0.00001 * self.cfg2.get_ph_units())), phasors
            )
        ...
        if analog and self.cfg2.get_num_pmu() > 1:  # Check if multistreaming:
            ...
            for i, df in self.cfg2.get_data_format():
                if not df[2]:  # Check if analog representation is integer
                    analog[i] = map(
                        lambda x: int(x / self.cfg2.get_analog_units()[i]), analog[i]
                    )
    ...
    def acceptor(self):
        while True:
            self.logger.info(
                "[%d] - Waiting for connection on %s:%d",
                self.cfg2.get_id_code(),
                self.ip,
                self.port,
            )
            ...
            indexes_to_pop = sorted(
                [i for i, c in enumerate(self.clients) if not c.is_alive()],
                reverse=True,
            )
            ...
            while True:
                ...
                if readable:
                    ...
                    if len(received_data) == total_frame_size:
                        try:
                            ...
                            logger.warning(
                                "[%d] - Received unknown message <- (%s:%d)",
                                pmu_id,
                                address[0],
                                address[1],
                            )
                        ...
                        logger.warning(
                            "[%d] - Message not received completely <- (%s:%d)",
                            pmu_id,
                            address[0],
                            address[1],
                        )
                ...
                if command:
                    if command == "start":
                        ...
                        logger.info(
                            "[%d] - Requested Header frame sent -> (%s:%d)",
                            pmu_id,
                            address[0],
                            address[1],
                        )
                        ...
                        logger.info(
                            "[%d] - Requested Configuration frame 1 sent -> (%s:%d)",
                            pmu_id,
                            address[0],
                            address[1],
                        )
                        ...
                        logger.info(
                            "[%d] - Requested Configuration frame 2 sent -> (%s:%d)",
                            pmu_id,
                            address[0],
                            address[1],
                        )
                        ...
                        logger.info(
                            "[%d] - Requested Configuration frame 3 sent -> (%s:%d)",
                            pmu_id,
                            address[0],
                            address[1],
                        )
                ...
                if sending_measurements_enabled and not buffer.empty():
                    ...
                    if isinstance(
                        data, CommonFrame
                    ):  # If not raw bytes convert to bytes
                        if set_timestamp:
                            data.set_time()
