...
class MarconaGrid(GridDefinition):
    ...
    def get_active_power(self, time: datetime) -> dict[str, str]:
        ...
        result["Datacenter1"] = normalize(power(time, 12).get("Datacenter1", 0.0))
        result["Datacenter2"] = normalize(power(time, 12).get("Datacenter2", 0.0))
        result["SECahuachi"] = normalize(power(time, 12).get("SECahuachi", 0.0))
        result["SEDerivacion"] = normalize(power(time, 12).get("SEDerivacion", 0.0))
        result["SEIca"] = normalize(power(time, 12).get("SEIca", 0.0))
        return result

    def get_reactive_power(self, time: datetime) -> dict[str, str]:
        result = super().get_reactive_power(time)
        result["SECahuachi"] = normalize(
            power(time, 12, reactive=True).get("SECahuachi", 0.0)
        )
        result["SEDerivacion"] = normalize(
            power(time, 12, reactive=True).get("SEDerivacion", 0.0)
        )
        result["SEIca"] = normalize(power(time, 12, reactive=True).get("SEIca", 0.0))
        return result
    ...
    def get_active_power_start(self, time: datetime) -> dict[str, str]:
        ...
        result["CAH-DER-0"] = normalize(
            power(time, 12, end=False).get("CAH-DER-0", 0.0)
        )
        result["CAH-DER-1"] = normalize(
            power(time, 12, end=False).get("CAH-DER-1", 0.0)
        )
        result["DER-ICA"] = normalize(power(time, 12, end=False).get("DER-ICA", 0.0))
        return result

    def get_active_power_end(self, time: datetime) -> dict[str, str]:
        result = super().get_active_power_end(time)
        result["CAH-DER-0"] = normalize(power(time, 12, end=True).get("CAH-DER-0", 0.0))
        result["CAH-DER-1"] = normalize(power(time, 12, end=True).get("CAH-DER-1", 0.0))
        result["DER-ICA"] = normalize(power(time, 12, end=True).get("DER-ICA", 0.0))
        return result
